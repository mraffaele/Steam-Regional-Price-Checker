...
class CacheDb {
  constructor(collection) {
    ...
    this.mongoUrl = `mongodb+srv://${Config.MONGO_USERNAME}:${Config.MONGO_PASSWORD}@${Config.MONGO_DOMAIN}/${Config.MONGO_DB}${Config.MONGO_OPTS}`;
  }
  ...
  save(data, id) {
    ...
    MongoClient.connect(this.mongoUrl, { useNewUrlParser: true }, (err, client) => {
      ...
      if (id) {
        collection.replaceOne({ _id: id }, savedData).then(() => {
          client.close();
        });
      } else {
        collection.insertOne(savedData, {}, () => client.close());
      }
    });
  }
  ...
  incrementHits(filter) {
    ...
    MongoClient.connect(this.mongoUrl, { useNewUrlParser: true }, (err, client) => {
      ...
      collection.update(filter, { $inc: { hits: 1 } }).then(() => client.close());
    });
  }
}
